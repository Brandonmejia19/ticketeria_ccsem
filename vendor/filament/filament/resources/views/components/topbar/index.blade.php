    ...
    <nav
                                ...
                                <x-filament::dropdown.list>
                                    @foreach ($group->getItems() as $item)
                                        ...
                                        <x-filament::dropdown.list.item :badge="$item->getBadge()" :badge-color="$item->getBadgeColor()"
                                            :href="$item->getUrl()" :icon="$item->isActive() ? $item->getActiveIcon() ?? $icon : $icon" tag="a" :target="$item->shouldOpenUrlInNewTab() ? '_blank' : null">
                                            {{ $item->getLabel() }}
        ...
        <div x-persist="topbar.end" class="ms-auto flex items-center gap-x-4">
            ...
            @if (filament()->auth()->check())
                ...
                <x-filament::dropdown placement="bottom-end">
                    <x-slot name="trigger">
                        <div class="p-3 rounded bg-gray-200 flex items-center">
                            <x-filament::icon-button icon="heroicon-s-user" label="Pausas" color="black" />
                            <span class="ml-2 text-bg-white-500 dark:text-primary-200">Pausas</span>
                        </div>
                    </x-slot>
                    <x-filament::dropdown.list>
                        <x-filament::dropdown.list.item href="#" icon="heroicon-o-clock">
                            Pausa 1
                        </x-filament::dropdown.list.item>
                        <x-filament::dropdown.list.item href="#" icon="heroicon-o-clock">
                            Pausa 2
                ...
                <x-filament::dropdown placement="bottom-end">
                    <x-slot name="trigger">
                        <x-filament::icon-button icon="heroicon-s-computer-desktop" label="Pausas" color="black" />
